...
def log_top_best_with_props(pairs, props, top_n, lgg):
    ...
    for i, ab in enumerate(all_best):
        molprops = props[ab[0]]
        lgg.info("%d. %s, %s, QED: %s, SA: %s, Dock: %s, mean similarity.: %s, max similarity.: %s, most similar: %s" %
...
for i in range(100000):
    try:
        ...
        tot_score = 0.40*qedscore + 0.10*sa_score + 0.50*norm_docking_score

        all_smiles[sanitized] = (tot_score, generated)

        mol_props[sanitized] = (qedscore, sa_score, docking_score, mean_distance, max_similarity, Chem.MolToSmiles(most_similar_submission_mol))
