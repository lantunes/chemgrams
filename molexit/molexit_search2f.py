...
for n in range(num_iterations):
    ...
    smiles = DeepSMILESLanguageModelUtils.sanitize(decoded)
    best_score = distance_scorer.score(smiles)
    logger.info("best SMILES: %s, J: %s (%s seconds)" % (smiles, best_score, str((end - start))))
    ...
    logger.info("evaluating LMs to see which is better...")
    existing_lm_evals = []
    while len(existing_lm_evals) != 50:
        try:
            generated = lm.generate(num_chars=50, text_seed='<s>')
            decoded = DeepSMILESLanguageModelUtils.decode(generated, start='<s>', end='</s>')
            smiles = DeepSMILESLanguageModelUtils.sanitize(decoded)
            existing_lm_evals.append(distance_scorer.score(smiles))
        except Exception:
            pass

    new_lm_evals = []
    while len(new_lm_evals) != 50:
        try:
            generated = new_lm.generate(num_chars=50, text_seed='<s>')
            decoded = DeepSMILESLanguageModelUtils.decode(generated, start='<s>', end='</s>')
            smiles = DeepSMILESLanguageModelUtils.sanitize(decoded)
            new_lm_evals.append(distance_scorer.score(smiles))
        except Exception:
            pass

    if np.mean(new_lm_evals) > np.mean(existing_lm_evals):
        logger.info("replacing LM...")
        lm = new_lm